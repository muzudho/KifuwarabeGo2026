...
| `KifuwarabeGo2026.Communication.Cgos` | コンソール通信クライアント | CGOS サーバーへ TCP 接続し、CGOS と GTP エンジンの間を中継 |
...
KifuwarabeGo2026
  ...
  -> 外部プロセスとして KifuwarabeGo2026.Communication.Cgos.exe を起動
...
1. リポジトリルートを探す。
2. `KifuwarabeGo2026.Communication.Cgos/bin/{Debug|Release}/net8.0/KifuwarabeGo2026.Communication.Cgos.exe` を探す。
...
`KifuwarabeGo2026.Communication.Cgos/Program.cs` の `CgosClientOptions.Parse(...)` がコマンドラインを読む。
...
`KifuwarabeGo2026.Communication.Cgos` 内の `GtpEngineProcess` が、`--engine-command` で受け取ったコマンドラインを起動する。
...
GUI からは `KifuwarabeGo2026.Communication.Cgos.exe` を直接探して起動する。`dotnet run` ではなくビルド済み exe 前提なので、GUI から CGOS を試す前に対象構成でビルドが必要。
...
| CGOS のログインや TCP 接続を変える | `KifuwarabeGo2026.Communication.Cgos/Program.cs` の `CgosConnectionSession`、`CgosTcpConnector` |
| CGOS コマンド対応を増やす | `KifuwarabeGo2026.Communication.Cgos/Program.cs` の `CgosClient.HandleLineAsync(...)` |
| GTP エンジン起動・応答読み取りを変える | `KifuwarabeGo2026.Communication.Cgos/Program.cs` の `GtpEngineProcess` |
