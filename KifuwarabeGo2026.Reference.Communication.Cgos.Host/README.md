...
```powershell
dotnet run --project KifuwarabeGo2026.Reference.Communication.Cgos.Host -- --account black
```

白番アカウントで接続:

```powershell
dotnet run --project KifuwarabeGo2026.Reference.Communication.Cgos.Host -- --account white
```

黒番・白番の両方で接続:

```powershell
dotnet run --project KifuwarabeGo2026.Reference.Communication.Cgos.Host -- --both
```
...
```powershell
dotnet run --project KifuwarabeGo2026.Reference.Communication.Cgos.Host -- --account black --engine-command ".\KifuwarabeGo2026.Reference.PlayerEngine.Go.Gtp.Host\bin\Release\net8.0\win-x64\publish\KifuwarabeGo2026.Engine.exe"
```
