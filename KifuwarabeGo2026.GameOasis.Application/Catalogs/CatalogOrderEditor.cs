...
/// <summary>カタログ一覧で共用する、保存前の順序編集状態です。</summary>
public sealed class CatalogOrderEditor<T>
{
    ...
    public void Cancel() { IsOpen = false; DraggedIndex = -1; }
    public IReadOnlyList<T> Commit() { IsOpen = false; DraggedIndex = -1; return _items.ToArray(); }
    public void Select(int index) { if (index >= 0 && index < _items.Count) SelectedIndex = index; }
    public void BeginDrag(int index) { Select(index); DraggedIndex = SelectedIndex; }

    public void DragTo(int index)
    {
        if (DraggedIndex < 0 || index < 0 || index >= _items.Count || index == DraggedIndex) return;
    ...
    public void MoveSelected(int offset) { if (SelectedIndex >= 0 && _items.Count > 0) MoveSelectedTo(Math.Clamp(SelectedIndex + offset, 0, _items.Count - 1)); }
    public void MoveSelectedToTop() => MoveSelectedTo(0);
    public void MoveVisiblePages(int offset) => FirstVisiblePageIndex = Math.Clamp(FirstVisiblePageIndex + offset, 0, PageCount - 1);

    private void MoveSelectedTo(int destination)
    {
        if (SelectedIndex < 0 || destination == SelectedIndex) return;
