...
public sealed class InstallerScreen : IDisposable
{
    ...
    private readonly IInstallerGuiPlatform _platform;
    private readonly IInstallerEngine _engine;
    private readonly KfwStationeryDrawingTools _draw;
    private readonly Action _exitApplication;
    private readonly object _stateGate = new();
    private readonly StationeryButton _start = new(new Rectangle(560, 280, 210, 66), "START LOBBY", 0.28f);
    private readonly StationeryButton _shortcut = new(new Rectangle(480, 785, 650, 66), "CREATE LOBBY DESKTOP SHORTCUT", 0.27f);
    ...
    public InstallerScreen(KfwStationeryDrawingTools drawingTools, IInstallerGuiPlatform platform, IInstallerEngine engine, Action exitApplication)
    {
        _draw = drawingTools;
    ...
    public void Update()
    {
        if (_engine is IInstallerEngineCommunicationStatus communication &&
    ...
    public void Draw()
    {
        ...
        _draw.DrawText("INSTALLER", new Vector2(124, 158), new Color(99, 223, 185), 0.38f);
    ...
    private void DrawMain(Point mouse)
    {
        ...
        SetButtonsEnabled(!_busy, _shortcut);
        _shortcut.Draw(mouse, _draw);
        _draw.DrawFittedText("S: START LOBBY   G: GUI UPDATE   E: ENGINE UPDATE   A: ALL   I: VERSIONS",
    ...
    private void DrawSettings(Point mouse)
    {
        ...
        _draw.DrawFittedText("CLOSE INSTALLER AFTER STARTING GUI",
            new Rectangle(240, 788, 780, 60), Color.White, 0.54f);
        _settingsBack.Draw(mouse, _draw);
        _draw.DrawFittedText("THIS SETTING IS SHARED BY THE INSTALLER AND EVERY GUI VERSION.",
    ...
    private void UpdateMain(Point point, bool click, KeyboardState keyboard, GamePadState gamePad)
    {
        ...
        else if (click && _shortcut.IsHit(point))
        {
            try { SetStatus(_platform.CreateLobbyShortcut()); }
            catch (Exception exception) { SetStatus("SHORTCUT FAILED: " + exception.Message); }
        }
        else if ((click && _guiUpdate.IsHit(point)) || Pressed(keyboard, Keys.G)) _ = UpdateProductAsync(InstallerProduct.Gui);
        else if ((click && _engineUpdate.IsHit(point)) || Pressed(keyboard, Keys.E)) _ = UpdateProductAsync(InstallerProduct.Engine);
    ...
    private void UpdateSettings(Point point, bool click, KeyboardState keyboard, GamePadState gamePad)
    {
        ...
        else if (click && new Rectangle(170, 778, 870, 80).Contains(point))
        {
            ...
            SetStatus(result.IsSuccess
                ? value ? "INSTALLER WILL CLOSE AFTER GUI START" : "INSTALLER WILL REMAIN OPEN AFTER GUI START"
    ...
    private async Task UpdateProductAsync(InstallerProduct product)
    {
        _busy = true;
        try
        {
            var progress = new InstallerProgressReporter(SetStatus);
    ...
    private async Task UpdateAllAsync() { await UpdateProductAsync(InstallerProduct.Gui); await UpdateProductAsync(InstallerProduct.Engine); }
    ...
    private void OpenCurrentEngine() { var directory = _engine.GetCurrentDirectory(InstallerProduct.Engine); SetStatus(directory is not null && _platform.OpenFolder(directory) ? "ENGINE FOLDER OPENED" : "ENGINE IS NOT INSTALLED"); }
    ...
    private sealed class InstallerProgressReporter(Action<string> report) : IProgress<InstallerProgress>
    {
        public void Report(InstallerProgress value) => report(value.Message);
    }
}
