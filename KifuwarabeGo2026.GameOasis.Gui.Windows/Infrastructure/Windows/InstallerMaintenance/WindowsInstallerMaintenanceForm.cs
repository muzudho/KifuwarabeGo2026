...
internal sealed class WindowsInstallerMaintenanceForm : Form
{
    private readonly WindowsInstallerPackageInstaller installer;
    private readonly WindowsInstallerShortcutStore store;
    ...
    private List<InstallerShortcutEntry> entries;

    public WindowsInstallerMaintenanceForm(
        WindowsInstallerPackageInstaller installer,
        WindowsInstallerShortcutStore store,
        WindowsShellLinkService shellLinks)
    {
        ...
        entries = store.Load();
        Text = "Kifuwarabe Go 2026 - インストーラー更新";
    ...
    private async Task InstallLatestAsync()
    {
        ...
        try
        {
            ...
            statusLabel.Text = $"配置完了: インストーラー v{result.Version}";
        }
        catch (Exception exception)
        {
            statusLabel.Text = "更新失敗: " + exception.Message;
            MessageBox.Show(this, exception.Message, "インストーラー更新失敗", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        finally
        {
            SetBusy(false);
    ...
    {
        if (entries.Count >= WindowsInstallerShortcutStore.MaximumCount)
        {
        ...
        {
            Title = "インストーラーのWindowsショートカットを選択してください",
        ...
        try
        {
            ...
            if (!KifuwarabeGo2026.InstallerEngine.Platform.WindowsLobbyShortcut.IsInstaller(target))
                throw new InvalidOperationException("選択したショートカットはKifuwarabeGo2026.Installer.exeを指していません。");
            entries.Add(new InstallerShortcutEntry(
    ...
    private void UpdateShortcutsInOrder()
    {
        ...
        {
            MessageBox.Show(this, "先に［最新版を取得して配置］を実行してください。", "インストーラー未配置", MessageBoxButtons.OK, MessageBoxIcon.Information);
            return;
        }
        var updated = 0;
        var skipped = 0;
        ...
        for (var index = 0; index < entries.Count; index++)
        {
            ...
            else
            {
                ...
                try
                {
                    shellLinks.RewriteInstallerTarget(entry.Path, entry.LastKnownTarget, installer.CurrentExecutable);
    ...
    private void StartCurrent()
    {
        try
        {
            ...
            statusLabel.Text = "新しいインストーラーを起動しました。";
        }
        catch (Exception exception)
        {
    ...
    private void RefreshState()
    {
        ...
        for (var index = 0; index < WindowsInstallerShortcutStore.MaximumCount; index++)
        {
        ...
        addButton.Enabled = entries.Count < WindowsInstallerShortcutStore.MaximumCount;
    }
}
