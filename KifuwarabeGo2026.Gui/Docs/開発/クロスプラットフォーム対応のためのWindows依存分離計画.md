...
### 2026-07-29: 移植性スモークを自動回帰検査へ拡張

- Coreの `TargetFrameworkAttribute` が `net8.0` であることを検査する。
- Coreの直接参照に `System.Windows.Forms` と `System.Drawing.Common` がないことを検査する。
- Core内のメソッドを調べ、P/Invoke実装が混入していないことを検査する。
- 移植用代替サービスのクリップボード、実行ファイル名、ファイルフィルター、文字画像、ページ数が安全な値を返すことを検査する。
- `Game1` の構成例が8個のプラットフォーム契約を満たしてコンパイルできることを検査する。
