...
- 大会ルールの `Id` は省略可能。省略時は内部IDを自動生成する。表示名を変えても同じルールとして更新・削除するための識別子なので、リリース既定値では読みやすい固定IDを付けてもよい。
- 大会ルールの `Rule` は `Chinese`、`Japanese`、`PureGo` のようにPascalCaseで記述する。旧来の小文字表記も互換読込する。
- 大会ルールの `MainTime` は `時:分:秒`。時は0～999、分と秒は0～59。旧 `MainTimeMinutes`、`MainTimeSeconds` も互換読込するが、新しく保存すると `MainTime` へ統一する。
