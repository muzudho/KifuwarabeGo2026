...
- [x] エントリープロファイル
  ...
  - 通常のプロファイル選択・編集も `ProfileSelection` と `ProfileEdit` に分け、固定操作領域と動的な一覧・入力欄を移管した。
...
- [x] Popup Trend Chart画面
  ...
  - 着手コメントパネルの表示切替を区画ラベルの任意ピンへ集約し、従来のCOMMENTSチェックを削除した。非表示時は画面左下に抜けたピン付き区画ラベルだけを表示する。
