...
| `label` | GUIに表示する名前 |
| `type` | オプションの型。現在は `check`、`spin`、`combo`、`string`、`filename` に対応 |
| `default` | 既定値 |
| `value` | エンジン起動時点の現在値 |
...
`button` は保存値ではなく、起動中のエンジンへその場でコマンドを送る特殊な型です。現在のKifuwarabe Go 2026は、エンジンプロファイルを編集して次回起動時に値を送る方式なので、`button` にはまだ対応していません。
