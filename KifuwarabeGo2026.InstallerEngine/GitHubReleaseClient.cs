...
internal sealed class GitHubReleaseClient(HttpClient client)
{
    ...
    public async Task<ReleasePackage> GetLatestAsync(InstallerProduct product, CancellationToken cancellationToken)
    {
        client.DefaultRequestHeaders.UserAgent.ParseAdd("KifuwarabeGo2026.Installer");
...
internal sealed record ReleasePackage(InstallerProduct Product, string Version, string AssetName, Uri DownloadUri, Uri? ChecksumUri);
