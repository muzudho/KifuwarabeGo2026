...
internal sealed class ProductLauncher(InstallerPaths paths, InstallerSettingsStore settingsStore, InstallerLog log, IPlatformProcessService platform)
{
    public LaunchResult StartGui()
    {
        ...
        return new(false, false, "起動できるロビーがありません。［GUI UPDATE］で最新版を取得してください。");
    }

    public string? CurrentDirectory(InstallerProduct product)
    {
    ...
    private LaunchResult TryStart(string? version)
    {
        ...
        var directory = paths.VersionDirectory(InstallerProduct.Gui, version);
        var executable = Path.Combine(directory, InstallerProduct.Gui.ExecutableName());
