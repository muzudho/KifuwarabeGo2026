...
internal sealed class InstalledVersionCatalog
{
    ...
    private readonly InstallerSettingsStore _settingsStore;
    private readonly IRunningProcessCatalog _runningProcesses;

    public InstalledVersionCatalog(InstallerPaths paths, InstallerSettingsStore settingsStore, IRunningProcessCatalog runningProcesses)
    {
        _paths = paths;
    ...
    public IReadOnlyList<InstalledVersion> ReadAll()
    {
        ...
        AddProductVersions(versions, InstalledProduct.Gui, _paths.ProductRoot(InstallerProduct.Gui), settings.GuiCurrentVersion, settings.GuiPreviousVersion);
        AddProductVersions(versions, InstalledProduct.Engine, _paths.ProductRoot(InstallerProduct.Engine), settings.EngineCurrentVersion, settings.EnginePreviousVersion);
    ...
    public void Uninstall(InstalledVersion installedVersion)
    {
        ...
        {
            InstalledProduct.Gui => _paths.ProductRoot(InstallerProduct.Gui),
            InstalledProduct.Engine => _paths.ProductRoot(InstallerProduct.Engine),
        ...
        if (!string.Equals(expectedPath, actualPath, StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException("インストーラーの管理フォルダー外は削除できません。");
