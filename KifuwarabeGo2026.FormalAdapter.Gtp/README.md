...
GTPの純粋なプロトコル契約、外部プロセスクライアント、オプション文書、囲碁座標変換をこの配下へ集約しています。`KifuwarabeGo2026.FormalAdapter.Gtp.PlayerEngine`は外部GTPエンジンをProtocol Pへ接続し、公式参照GTPサーバーは`KifuwarabeGo2026.Reference.PlayerEngine.Go.Gtp`が所有します。`KifuwarabeGo2026.Reference.PlayerEngine.Go.GtpExtensions`には初期局面戦略と能力プロファイルを残し、実行ファイル名、設定、発行物との互換性を維持します。
