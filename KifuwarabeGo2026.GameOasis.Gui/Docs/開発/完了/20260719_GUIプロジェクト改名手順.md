...
- フォルダー: `KifuwarabeGo2026` → `KifuwarabeGo2026.GameOasis.Gui`
- プロジェクトファイル: `KifuwarabeGo2026.csproj` → `KifuwarabeGo2026.GameOasis.Gui.csproj`
- 出力ファイル: `KifuwarabeGo2026.exe` → `KifuwarabeGo2026.GameOasis.Gui.exe`

`KifuwarabeGo2026.Engine` と `KifuwarabeGo2026.Reference.Communication.Cgos.Host` は改名しません。C# の名前空間も今回は `KifuwarabeGo2026` のままとします。名前空間まで一緒に変えると変更範囲が大きくなり、プロジェクト名を分かりやすくする目的とは別の作業になるためです。
...
```powershell
Rename-Item -LiteralPath '.\KifuwarabeGo2026' -NewName 'KifuwarabeGo2026.GameOasis.Gui'
```

確認します。

```powershell
Get-Item -LiteralPath '.\KifuwarabeGo2026.GameOasis.Gui' | Select-Object FullName
...
```text
KifuwarabeGo2026.GameOasis.Gui\Docs\開発\完了\20260719_GUIプロジェクト改名手順.md
```
...
```powershell
Rename-Item -LiteralPath '.\KifuwarabeGo2026.GameOasis.Gui\KifuwarabeGo2026.csproj' -NewName 'KifuwarabeGo2026.GameOasis.Gui.csproj'
```

確認します。

```powershell
Get-Item -LiteralPath '.\KifuwarabeGo2026.GameOasis.Gui\KifuwarabeGo2026.GameOasis.Gui.csproj' | Select-Object FullName
```
...
`AssemblyName` により exe 名が `KifuwarabeGo2026.GameOasis.Gui.exe` になります。`RootNamespace` は既存の C# 名前空間を維持する意図を明示します。
...
2. `KifuwarabeGo2026.Engine/KifuwarabeGo2026.Engine.csproj`
   ...
   - → `..\KifuwarabeGo2026.GameOasis.Gui\Domain\*.cs`
3. `tools/Generate-ExecutableIcons.ps1`
   - GUI アイコンの出力先を `KifuwarabeGo2026.GameOasis.Gui\GuiIcon.ico` にする
4. ルートの `README.md`
...
```powershell
Get-ChildItem -LiteralPath '.\KifuwarabeGo2026.GameOasis.Gui\bin', '.\KifuwarabeGo2026.GameOasis.Gui\obj' -ErrorAction SilentlyContinue |
    Select-Object FullName
```

表示された場所が `KifuwarabeGo2026.GameOasis.Gui` 内の `bin` と `obj` だけであることを確認してから、Codex に削除を依頼します。手作業で削除する場合も、リポジトリ直下やプロジェクト本体を対象にしてはいけません。
...
- 警告 0
- エラー 0
- `KifuwarabeGo2026.GameOasis.Gui.exe` が生成される
...
Get-Item -LiteralPath `
    '.\KifuwarabeGo2026.GameOasis.Gui\bin\Debug\net8.0-windows\KifuwarabeGo2026.GameOasis.Gui.exe', `
...
```powershell
dotnet run --project '.\KifuwarabeGo2026.GameOasis.Gui\KifuwarabeGo2026.GameOasis.Gui.csproj'
```
...
- ソリューション上の GUI プロジェクト名が `KifuwarabeGo2026.GameOasis.Gui`
- GUI の出力名が `KifuwarabeGo2026.GameOasis.Gui.exe`
