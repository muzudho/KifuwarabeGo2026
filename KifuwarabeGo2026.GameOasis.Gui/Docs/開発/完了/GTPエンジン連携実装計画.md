...
KifuwarabeGo2026.GameOasis.Gui.Gtp             GTP通信・座標変換・プロセス管理
KifuwarabeGo2026.Engine          別プロセスの思考エンジン
```

最初は `KifuwarabeGo2026.GameOasis.Gui.Gtp` を別プロジェクトにせず GUI プロジェクト内のフォルダーで始めてもよい。ただし、責務は最初から分けておく。
...
- 投了、二連続パス、コンピューター着手による終局時に GTP エンジンプロセスを閉じる。
...
状態: 基本実装済み。終局時のエンジン終了処理も追加済み。次は異常系と長時間対局の確認を厚くする。
...
- 人間が着手、パスした場合は、必要に応じてエンジンへ `play` を送って同期する。
- 投了時は GTP へ専用コマンドを送らず、GUI 側で終局してエンジンプロセスを閉じる。
...
6. パスと投了の結果を GUI とエンジンの間で破綻なく扱える。状態: 完了。投了時はエンジンプロセスを閉じる。
...
- 終局時の GTP エンジン終了処理追加後、`dotnet build KifuwarabeGo2026.slnx` 成功。
