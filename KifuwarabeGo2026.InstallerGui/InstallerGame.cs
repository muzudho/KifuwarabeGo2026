...
internal sealed class InstallerGame : Game
{
    ...
    public InstallerGame(IInstallerGuiPlatform platform, IInstallerEngine engine, string? startupMessage = null, EventWaitHandle? activation = null)
    {
        _platform = platform;
        _engine = engine;
    ...
    {
        var version = typeof(InstallerGame).Assembly.GetName().Version;
        return version is null
            ? "Kifuwarabe Go 2026 Installer"
            : $"Kifuwarabe Go 2026 Installer | v{version.Major}.{version.Minor}.{version.Build}";
    }
    ...
    protected override void LoadContent()
    {
        ...
        if (_startupMessage is not null) _screen.ShowStatus(_startupMessage);
    ...
    {
        if (_activation?.WaitOne(0) == true)
            _screen?.ShowStatus("INSTALLER READY: USE START LOBBY OR CHECK GUI + ENGINE UPDATES.");
