...
`codex_apps` は警告に含まれるが、Apps を無効化しても ReactiveMemory と Developer Docs の初期化は完了しない。  
各 MCP を直接起動または HTTPS 接続すると基本的な可用性は確認できるため、Codex CLI v0.146.0 の MCP クライアント起動、起動タイムアウト、環境継承、またはプロキシ処理に原因がある可能性が高い。
...
- グローバルにインストールされているパッケージは `@openai/codex@0.146.0`。
- npm レジストリで確認できた最新バージョンも `0.146.0`。
- この時点では、CLI の更新による回避はできない。

## 設定変更の方針

ユーザーの指定により、MCP を恒久的に無効化する設定変更は行わない。警告を回避するための候補はあるが、Apps、ReactiveMemory、Developer Docs の機能が利用できなくなるため、必要性を確認できるまで適用しない。
