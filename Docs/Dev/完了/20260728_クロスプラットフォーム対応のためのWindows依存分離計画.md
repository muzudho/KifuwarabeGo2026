...
- Windows 実装のプロジェクト名は `KifuwarabeGo2026.GameOasis.Gui.Windows` を第一候補とする。
- 将来は `KifuwarabeGo2026.GameOasis.Gui.Linux`、`KifuwarabeGo2026.GameOasis.Gui.Mac` を同じ並びで追加できる形にする。
- Windows 実装を別プロジェクトへ移した後も、`AssemblyName` を `KifuwarabeGo2026.GameOasis.Gui` に指定し、利用者が起動するファイル名を `KifuwarabeGo2026.GameOasis.Gui.exe` のまま維持する。
...
- 実行中の `KifuwarabeGo2026.GameOasis.Gui.exe` が通常のDebug出力先をロックしていたため、TEMP内の別出力先へGUIプロジェクトをビルドし、警告0、エラー0を確認した。
...
これでWindows固有サービスの境界作りは一巡した。まだ物理的なプロジェクト分割は行っていない。次はパスやフィルターに残るWindows前提を整理し、`KifuwarabeGo2026.GameOasis.Gui` と `KifuwarabeGo2026.GameOasis.Gui.Windows` を作る前の最終監査を行う。
...
- `KifuwarabeGo2026.GameOasis.Gui` を参照する。
- `AssemblyName=KifuwarabeGo2026.GameOasis.Gui` とし、実行ファイル名を `KifuwarabeGo2026.GameOasis.Gui.exe` にする。
...
最初の分割では既存の `KifuwarabeGo2026.GameOasis.Gui/Content` を移動せず、Windows csprojからリンクまたは明示参照する。これにより大量のアセット移動を避け、`Content.RootDirectory = "Content"` と実行時の相対配置を維持する。

Core単体でContentをビルドする必要はない。将来Linux/macOS起動プロジェクトも同じContent定義を参照する。

### ソース配置

大量の共通ソースを一度に移動しないため、当初は現在の `KifuwarabeGo2026.GameOasis.Gui` ディレクトリをCoreのソース置場として利用する。

```text
KifuwarabeGo2026.GameOasis.Gui/
    KifuwarabeGo2026.GameOasis.Gui.csproj
    Game1.cs
    Application/
    Gtp/
    Presentation/
...
KifuwarabeGo2026.GameOasis.Gui.Windows/
    KifuwarabeGo2026.GameOasis.Gui.Windows.csproj
    Program.cs
    Infrastructure/Windows/
    app.manifest
    GuiIcon.ico
...
Windows専用ソースを新プロジェクトへ移してから、旧 `KifuwarabeGo2026.GameOasis.Gui.csproj` をCore用csprojへ置換する。既存のビルド・publish手順はWindowsプロジェクトを指すよう更新する。

### 2026-07-28: Core／Windowsの物理的なプロジェクト分割を完了

- `KifuwarabeGo2026.GameOasis.Gui.csproj` を追加し、共通GUIを `net8.0` のライブラリにした。
- `KifuwarabeGo2026.GameOasis.Gui.Windows.csproj` を追加し、Windows起動側を `net8.0-windows` の `WinExe` にした。
- `Program.cs` と `Infrastructure/Windows` の全実装をWindowsプロジェクトへ移した。
- Windowsプロジェクトの `AssemblyName` を `KifuwarabeGo2026.GameOasis.Gui` とし、実行ファイル名を維持した。
...
- `win-x64` publishが成功し、`KifuwarabeGo2026.GameOasis.Gui.exe`、`KifuwarabeGo2026.GameOasis.Gui.dll`、Content、`Tools/Cgos` を確認した。
...
- 新WindowsプロジェクトのDebug版 `KifuwarabeGo2026.GameOasis.Gui.exe` を起動した。
- 応答中のタイトル画面、Content読込、クリック入力ログ、例外0件を確認した。
- ログ保存先が分割前と同じ `Logs/Gui` であることを確認した。
- 詳細と残る画面操作は [Core・Windows分割後スモークテスト.md](./20260729_Core・Windows分割後スモークテスト.md) に記録した。

### 2026-07-29: 移植者向けスモークプロジェクトを追加

- `KifuwarabeGo2026.GameOasis.Gui.PortabilitySmoke` を `net8.0` のコンソールプロジェクトとして追加した。
...
- `dotnet run --project KifuwarabeGo2026.GameOasis.Gui.PortabilitySmoke\KifuwarabeGo2026.GameOasis.Gui.PortabilitySmoke.csproj --no-build` が成功した。
...
- `KifuwarabeGo2026.Tests.GameOasis.Gui.Windows` を `net8.0-windows` のコンソールプロジェクトとして追加した。
...
- Windows起動アセンブリ名が `KifuwarabeGo2026.GameOasis.Gui` のままであることを確認する。
...
将来追加:
KifuwarabeGo2026.GameOasis.Gui.Linux
KifuwarabeGo2026.GameOasis.Gui.Mac
```

既存の `KifuwarabeGo2026.GameOasis.Gui` を一度に改名・分割すると、MonoGame Content、publish、CGOS 同梱、実行ファイル名へ同時に影響する。このため、サービス境界を作ってからプロジェクトを分ける。
...
1. `KifuwarabeGo2026.GameOasis.Gui.Windows` を追加する。
2. Windows 実装、エントリーポイント、専用リソースを移す。
3. Windows プロジェクトだけを `net8.0-windows`、`UseWindowsForms=true` とする。
4. 共通 GUI コードを `net8.0` の `KifuwarabeGo2026.GameOasis.Gui` へ移す。
...
```xml
<AssemblyName>KifuwarabeGo2026.GameOasis.Gui</AssemblyName>
```

これにより、プロジェクト名が `KifuwarabeGo2026.GameOasis.Gui.Windows` でも、利用者が起動するファイルは `KifuwarabeGo2026.GameOasis.Gui.exe` になる。
...
- Windows 実装が `KifuwarabeGo2026.GameOasis.Gui.Windows` にある。
...
Windows版の確認は完了した。Linux/macOS向けの起動プロジェクト、OS固有実装、SDL2アイコン調査は [Linux・macOS GUI移植計画](../Archive/GuiPortingPlan.md) へ切り出した。

## 引継ぎ時の注意

- 現在の正式対応 OS は Windows であり、Linux/macOS 対応済みとは記載しない。
- GUIのCGOS同梱処理は `KifuwarabeGo2026.GameOasis.Gui.Windows.csproj` にある。
- WinForms 型をインターフェースの引数や戻り値へ含めない。
- `Game1` は大きいため、サービス単位の小さな変更を優先する。
- Linux/macOS移植の判断変更や調査結果は、現役の移植計画へ追記する。
