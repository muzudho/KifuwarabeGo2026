...
- 共有ログ `logs/gtp.log` を維持した。
- 黒用クライアントは `[black-engine]`、白用クライアントは `[white-engine]` の接頭辞を付けてログへ出すようにした。
- 黒用・白用クライアントが同じログファイルを開けるよう、ログファイルは共有読み書きで開く。
- 画面上のエラー文言に黒白を含める細分化は後続作業。
...
- セットアップ画面で黒番・白番を `Computer` にした場合、`Kifuwarabe Random GTP` を表示するようにした。
- 現時点では表示のみで、エンジン選択 UI はまだ作っていない。
...
- [続きはここから.md](../Archive/HandoffHistory.md)
