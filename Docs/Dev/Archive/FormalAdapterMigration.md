...
> 現状と後続作業は [FormalAdapter移行の到達点](../CurrentState/FormalAdapterMigration.md) / [FormalAdapter移行後の最終検証](../Plans/FormalAdapterValidation.md) を参照してください。
...
SGF文書パーサーはGUI、MonoGame、ファイルシステムへ本質的に依存しないため、最初の切り出し候補です。既存の[`SGF文書モデル実装計画.md`](SgfDocumentPlan.md)にある、未知プロパティと値順序を保持する`SgfDocument`、`SgfGameTree`、`SgfNode`、`SgfProperty`を`FormalAdapter.Sgf`へ実装する方針とします。
...
この節は[内容別の文書](../CurrentState/FormalAdapterMigration.md)へ移しました。
...
この節は[内容別の文書](../Plans/FormalAdapterValidation.md)へ移しました。
