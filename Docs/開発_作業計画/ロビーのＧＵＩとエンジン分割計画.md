...
状態：作業段階２完了、作業段階３の開始待ち（2026年8月28日）
...
現在の段階：作業段階３
現在の状態：未着手
次に行う作業：START、EDIT BOARD、ポン抜き開始が現在生成・参照する値を列挙し、最小のPlayRoomLaunchRequestと結果型を定める
最後に成功した検証：2026年8月28日、Releaseソリューションビルドは警告0・エラー0。LobbyEngine境界、LobbyGui境界、GameOasis関連テストがすべてPASS
現在有効な経路：単一 GameOasis.Gui 実行ファイル内で LobbyGuiController から InProcessLobbyEngine を利用し、表示用コピーを既存 GoAppSession へ投影する経路
ロールバック：Game1でLobbyGuiControllerが行う構成を第1段階末のILobbyEngine直接利用へ戻す。保存形式の巻き戻しは不要
未解決事項：PlayRoomLaunchRequestの所有プロジェクト、GameOasisGuiCompositionのロビー／プレイルーム分解、初期局面と参加者設定の契約表現
```
