...
状態：作業段階５完了、作業段階６の再計画待ち（2026年8月28日）
...
現在の段階：作業段階６
現在の状態：再計画待ち
次に行う作業：第5段階のlistEntries通信境界と復旧方式を前提に、最初に独立させるプレイルーム1種類と、起動・準備完了・終了・ロビー復帰の最小プロトコルを決める
最後に成功した検証：2026年8月28日、Releaseソリューションビルドは警告0・エラー0。実子プロセスとのlistEntries往復、起動失敗時の同一プロセス復旧、GameOasis Contracts、LobbyGui・PlayRoom境界を含む移植性テストはPASS
現在有効な経路：発行物にLobbyEngine.JsonLinesHostがあれば登録済み参加者一覧だけを標準入出力JSON Linesで読み、障害時と変更操作はInProcessLobbyEngineへ戻る。プレイルーム起動は従来どおり同一プロセスアダプターを利用する
ロールバック：LobbyGuiControllerからJsonLinesLobbyEngineの構成を外し、GameOasis.GuiのJsonLines参照、発行スクリプトのLobbyEngineホスト発行、追加2プロジェクトを除く。既存カタログと保存形式の巻き戻しは不要
未解決事項：実画面スモーク確認、変更操作の再実行安全性、ロビー全状態の通信化、最初に別実行ファイル化するプレイルーム、GameOasisGuiCompositionの分解、プレイルーム終了通知
```
