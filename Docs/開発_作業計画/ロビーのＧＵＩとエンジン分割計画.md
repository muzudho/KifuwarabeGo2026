...
状態：作業段階４実装完了、代表プレイルームの手動スモーク確認待ち（2026年8月28日）
...
```text
現在の段階：作業段階４
現在の状態：実装完了、手動スモーク確認待ち
次に行う作業：既存GUIを起動し、ローカル対局START、EDIT BOARD、ポン抜きへ入り、それぞれロビーへ戻れることを手動確認する
最後に成功した検証：2026年8月28日、Releaseソリューションビルドは警告0・エラー0。GameOasis Contracts、LobbyEngine、LobbyGui・PlayRoom境界を含む移植性テストはPASS
現在有効な経路：単一 GameOasis.Gui 実行ファイル内で、LobbyGui.ApplicationからLobbyEngineを利用し、PlayRoom.Launchingの起動契約と同一プロセスアダプターを介して既存プレイルームへ入る経路
ロールバック：LobbyGui/ApplicationとPlayRoom/Launchingのファイルを旧Application/Lobby、Application/PlayRoomへ戻し、Game1とテストのusingを旧名前空間へ戻す。実行ファイル名、発行経路、保存形式の巻き戻しは不要
未解決事項：実画面スモーク確認、GameOasisGuiCompositionのロビー／プレイルーム分解、プレイルーム終了通知の発行元、別実行ファイル化時のGameOasis.Guiシェル分割
```
