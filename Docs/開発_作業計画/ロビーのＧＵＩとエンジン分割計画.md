...
状態：作業段階３完了、作業段階４の開始待ち（2026年8月28日）
...
状態：今回の対象外。作業段階５の運用結果を確認した後に判断する
...
現在の段階：作業段階４
現在の状態：未着手
次に行う作業：GameOasis.Concierge、Application、Storage、Contracts、Gui の参照関係と実行ファイル名を再調査し、改名・移動する範囲と据え置く範囲を決める
最後に成功した検証：2026年8月28日、Releaseソリューションビルドは警告0・エラー0。GameOasis ContractsテストはPASS。追加した移植性テストはビルド成功したが、実行はWindowsアプリケーション制御ポリシー0x800711C7に遮断された
現在有効な経路：単一 GameOasis.Gui 実行ファイル内で、LobbyGuiControllerからInProcessLobbyEngineを利用し、START・EDIT BOARD・ポン抜き開始はPlayRoomLaunchRequestをInProcessPlayRoomLauncherへ渡す経路
ロールバック：Game1の3つの起動箇所を旧StartLocalMatchCore、具象ポン抜き生成、具象盤面編集生成へ戻し、Application/PlayRoomとContracts/PlayRoomを除く。保存形式の巻き戻しは不要
未解決事項：GameOasisGuiCompositionのロビー／プレイルーム分解、プレイルーム終了通知の発行元、作業段階4で行うプロジェクト改名・移動の範囲、実画面スモークテスト
```
