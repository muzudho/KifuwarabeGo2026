...
要求と応答は UTF-8 の JSON Lines とし、`protocolVersion`、`requestId`、操作名、成功状態、結果またはエラーを持つ。標準出力はプロトコル専用、標準エラー出力は診断ログ専用とした。詳細は `Docs/設計/ランチャーエンジンJSON Linesプロトコル.md` に記載した。
...
現行ロビー領域の調査結果と段階計画は、`Docs/開発_作業計画/ロビーのＧＵＩとエンジン分割計画.md` へ分離した。以後の実装は同文書を基準に１段階ずつ進める。
